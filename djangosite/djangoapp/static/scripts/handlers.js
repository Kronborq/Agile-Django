...
function uploadProgress(fileObj, bytesLoaded) {
	...
	try {
		...
		if (percent === 100) {
			progress.SetStatus("Opretter thumbnail...");
			progress.ToggleCancel(false);
			progress.ToggleCancel(true, this, fileObj.id);
		} else {
			progress.SetStatus("Uploader...");
			progress.ToggleCancel(true, this, fileObj.id);
		}
	} catch (ex) { this.debug(ex); }
}
...
function uploadSuccess(fileObj, server_data) {
	try {
		...
		AddImage("scripts/thumbnail.php?id=" + server_data);

		var progress = new FileProgress(fileObj,  this.customSettings.upload_target);

		progress.SetStatus("Thumbnail oprettet.");
		progress.ToggleCancel(false);
...
function uploadComplete(fileObj) {
	try {
		...
		if (this.getStats().files_queued > 0) {
			...
			progress.SetStatus("Alle billeder er uploaded (tryk evt F5 for at reloade).");
			progress.ToggleCancel(false);
		}
	} catch (ex) { this.debug(ex); }
}
